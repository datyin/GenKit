...
/**
 * Remove the role assignments for the specified item.
 *
 ...
 * ```
 */
async function removeItemRoleAssignments(item: IItem, userId: number, roleDefinitionId: number, clearExisting: boolean = true) {
  try {
    if (getBoolean(clearExisting, { default: true })) {
      await item.breakRoleInheritance(true);
    }

    await item.roleAssignments.remove(userId, roleDefinitionId);
...
export { removeItemRoleAssignments };
